...
/**
 * \brief Represents an OBDII response, obtained through a call to `OBDIIDecodeResponseForRequest` or `OBDIIPerformQuery()`.
 *
...
/**
 * \brief Type for a function that can decode the raw response payload for a particular command.
 * 
...
/**
 * \brief Represents an OBDII command (request for diagnostic data).
 *
...
typedef struct OBDIICommand {
	char *name;                              /*!< A human-readable description of the command. */
	unsigned char payload[2];                /*!< The raw request payload, which for all commands is simply a (mode, PID) tuple. */
	OBDIIResponseType responseType;	         /*!< The type of data contained in the response for this command. */
	short expectedResponseLength;            /*!< The expected length of a payload containing the raw response to this command. Equal to
                                                    * `VARIABLE_RESPONSE_LENGTH` for commands whose response payloads have variable length. */
	OBDIIResponseDecoder responseDecoder;    /*!< A pointer to a function that can decode a raw response payload for this command. */
} OBDIICommand;
...
/**
 * \brief Represents a dynamic collection of commands.
 *
...
/**
 * \brief Decode the raw response payload for a given command.
 *
 ...
 *          Make sure to call `OBDIIResponseFree()` when you are done with the
 *          response.
 */
OBDIIResponse OBDIIDecodeResponseForCommand(OBDIICommand *command, unsigned char *responsePayload, int len);

/**
 * \brief Free any resources allocated to this response object.
 *
...
/**
 * \brief Checks if a given command is present within a command set.
 *
...
/**
 * \brief Free any resources allocated to this command set.
 *
