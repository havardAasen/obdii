...
/**
 * \brief Open a communication channel to a particular ECU.
 *
 ...
 * \param shared If `1`, the socket will be opened by the OBDII daemon instead
 *               of by the calling process. This allows multiple processes to
 *               share the same OBDII socket, as only a single process can bind
 *               to a particular (interface, transfer ID, receive ID) tuple. If
 *               `0`, the calling process will bind the socket, which may fail
 *               if another process has already bound a socket for the given
 *               parameters.
 *
...
/**
 * \brief Close an open socket created with `OBDIIOpenSocket()`
 *
...
/**
 * \brief Query the car for a particular command and return the response.
 *
...
/**
 * \brief Queries the car for the commands it supports.
 *
 * \code
